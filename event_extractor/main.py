...
def stanfordcorenlp_command(path2corenlp, inputfile, memory='16g', use_server=True,
                            server_address="http://140.109.19.191:9000") -> str:
    if use_server:
        from pathlib import Path
        print('Running CoreNLP on', server_address)
        outputfile = str(Path(inputfile).with_suffix('.json').name)
        return 'wget --post-file {} \'{}/?properties={{"outputFormat":"json","annotators":"tokenize,ssplit,pos,depparse,lemma,ner,parse","ner.applyFineGrained":"false"}}\' -O {}'.format(
            inputfile, server_address, outputfile)
    else:
        return "java -cp " + os.path.join(path2corenlp,
...
def corenlp_gen_data(docID_list) -> dict:
    ...
    for docID in docID_list:
        lprint('docID:', docID)
        cmd = stanfordcorenlp_command(path2corenlp,
            ...
            for docID in docID_list:
                ...
                for sentID in range(0, len(data['sentences'])):
                    ...
                    for _, val in enumerate(sent_em):
                        line = "{}\t{}\t{}\t{},{}\n".format(val['ner'], docID, sentID, val['tokenBegin'],
                                                            val['tokenEnd'] - 1)
                        f.write(line)
